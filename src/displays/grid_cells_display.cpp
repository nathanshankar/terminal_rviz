...
GridCellsDisplay::GridCellsDisplay(rclcpp::Node::SharedPtr node)
    : Display("GridCells", node) {}
...
void GridCellsDisplay::setTopic(const std::string& topic) {
    ...
    TopicConfig cfg;
    cfg.color_index = 2; // Green default
    cfg.size = 0.2f;
    configs_[topic] = cfg;
    
    subs_[topic] = node_->create_subscription<nav_msgs::msg::GridCells>(
        topic, 10, [this, topic](const nav_msgs::msg::GridCells::SharedPtr msg) {
            std::lock_guard<std::mutex> lock(mtx_);
            latest_msgs_[topic] = msg;
        });
}

bool GridCellsDisplay::isTopicEnabled(const std::string& topic) const {
    std::lock_guard<std::mutex> lock(mtx_);
    return std::find(enabled_topics_.begin(), enabled_topics_.end(), topic) != enabled_topics_.end();
}

TopicConfig GridCellsDisplay::getTopicConfig(const std::string& topic) {
    std::lock_guard<std::mutex> lock(mtx_);
    if (configs_.count(topic)) return configs_[topic];
    return TopicConfig();
}

void GridCellsDisplay::setTopicConfig(const std::string& topic, const TopicConfig& config) {
    std::lock_guard<std::mutex> lock(mtx_);
    configs_[topic] = config;
}

void GridCellsDisplay::render(RvizRenderer& renderer, ftxui::Canvas&, const std::string& fixed_frame, std::shared_ptr<tf2_ros::Buffer> tf_buffer) {
    if (!enabled_) return;
    ...
    for (const auto& topic : topics) {
        nav_msgs::msg::GridCells::SharedPtr msg;
        TopicConfig cfg;
        {
            std::lock_guard<std::mutex> lock(mtx_);
        ...
        for (const auto& pt : msg->cells) {
            tf2::Vector3 p = frame_to_world * tf2::Vector3(pt.x, pt.y, pt.z);
            renderer.draw_tile(p.x(), p.y(), p.z(), msg->cell_width, msg->cell_height, r_c, g_c, b_c, cfg.alpha);
        }
    }
}

ftxui::Element GridCellsDisplay::render_2d(bool /*nav2_active*/, int config_scroll) {
    ...
    return vbox({
        hbox({ text(" GridCells Settings ") | bold | color(Color::Yellow), filler() }),
        separator(),
        vbox(std::move(topics_ui)) | size(HEIGHT, EQUAL, 10),
    }) | border;
}

std::vector<std::string> GridCellsDisplay::getEnabledTopics() const {
    std::lock_guard<std::mutex> lock(mtx_);
    return enabled_topics_;
}

bool GridCellsDisplay::handle_event(ftxui::Event /*event*/, int /*scroll_offset*/) {
    return false;
}
