#ifndef TERMINAL_RVIZ_LEGACY_POINTCLOUD_DISPLAY_HPP_
#define TERMINAL_RVIZ_LEGACY_POINTCLOUD_DISPLAY_HPP_
...
class LegacyPointCloudDisplay : public Display {
public:
    explicit LegacyPointCloudDisplay(rclcpp::Node::SharedPtr node);
    ...
    std::string getMessageType() const override { return "sensor_msgs/msg/PointCloud"; }

private:
    void callback(const sensor_msgs::msg::PointCloud::SharedPtr msg, const std::string& topic);

    mutable std::mutex mtx_;
    std::vector<std::string> enabled_topics_;
    std::map<std::string, rclcpp::Subscription<sensor_msgs::msg::PointCloud>::SharedPtr> subs_;
    std::map<std::string, sensor_msgs::msg::PointCloud::SharedPtr> latest_msgs_;
    std::map<std::string, TopicConfig> configs_;
};

} // namespace terminal_rviz

#endif // TERMINAL_RVIZ_LEGACY_POINTCLOUD_DISPLAY_HPP_
