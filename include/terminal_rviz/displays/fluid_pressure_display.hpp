#ifndef TERMINAL_RVIZ_FLUID_PRESSURE_DISPLAY_HPP_
#define TERMINAL_RVIZ_FLUID_PRESSURE_DISPLAY_HPP_
...
class FluidPressureDisplay : public Display {
public:
    explicit FluidPressureDisplay(rclcpp::Node::SharedPtr node);
    ...
    std::string getMessageType() const override { return "sensor_msgs/msg/FluidPressure"; }

private:
    void callback(const sensor_msgs::msg::FluidPressure::SharedPtr msg, const std::string& topic);

    mutable std::mutex mtx_;
    std::vector<std::string> enabled_topics_;
    std::map<std::string, rclcpp::Subscription<sensor_msgs::msg::FluidPressure>::SharedPtr> subs_;
    std::map<std::string, sensor_msgs::msg::FluidPressure::SharedPtr> latest_msgs_;
    std::map<std::string, TopicConfig> configs_;
};

} // namespace terminal_rviz

#endif // TERMINAL_RVIZ_FLUID_PRESSURE_DISPLAY_HPP_
