#ifndef TERMINAL_RVIZ_RELATIVE_HUMIDITY_DISPLAY_HPP_
#define TERMINAL_RVIZ_RELATIVE_HUMIDITY_DISPLAY_HPP_
...
class RelativeHumidityDisplay : public Display {
public:
    explicit RelativeHumidityDisplay(rclcpp::Node::SharedPtr node);
    ...
    std::string getMessageType() const override { return "sensor_msgs/msg/RelativeHumidity"; }

private:
    void callback(const sensor_msgs::msg::RelativeHumidity::SharedPtr msg, const std::string& topic);

    mutable std::mutex mtx_;
    std::vector<std::string> enabled_topics_;
    std::map<std::string, rclcpp::Subscription<sensor_msgs::msg::RelativeHumidity>::SharedPtr> subs_;
    std::map<std::string, sensor_msgs::msg::RelativeHumidity::SharedPtr> latest_msgs_;
    std::map<std::string, TopicConfig> configs_;
};

} // namespace terminal_rviz

#endif // TERMINAL_RVIZ_RELATIVE_HUMIDITY_DISPLAY_HPP_
