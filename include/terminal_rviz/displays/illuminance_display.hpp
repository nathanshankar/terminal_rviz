#ifndef TERMINAL_RVIZ_ILLUMINANCE_DISPLAY_HPP_
#define TERMINAL_RVIZ_ILLUMINANCE_DISPLAY_HPP_
...
class IlluminanceDisplay : public Display {
public:
    explicit IlluminanceDisplay(rclcpp::Node::SharedPtr node);
    ...
    std::string getMessageType() const override { return "sensor_msgs/msg/Illuminance"; }

private:
    void callback(const sensor_msgs::msg::Illuminance::SharedPtr msg, const std::string& topic);

    mutable std::mutex mtx_;
    std::vector<std::string> enabled_topics_;
    std::map<std::string, rclcpp::Subscription<sensor_msgs::msg::Illuminance>::SharedPtr> subs_;
    std::map<std::string, sensor_msgs::msg::Illuminance::SharedPtr> latest_msgs_;
    std::map<std::string, TopicConfig> configs_;
};

} // namespace terminal_rviz

#endif // TERMINAL_RVIZ_ILLUMINANCE_DISPLAY_HPP_
