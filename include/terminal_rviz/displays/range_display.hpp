#ifndef TERMINAL_RVIZ_DISPLAYS_RANGE_DISPLAY_HPP_
#define TERMINAL_RVIZ_DISPLAYS_RANGE_DISPLAY_HPP_
...
class RangeDisplay : public Display {
public:
    explicit RangeDisplay(rclcpp::Node::SharedPtr node);
    ...
    std::string getMessageType() const override { return "sensor_msgs/msg/Range"; }

private:
    void callback(const sensor_msgs::msg::Range::SharedPtr msg, const std::string& topic);

    std::map<std::string, rclcpp::Subscription<sensor_msgs::msg::Range>::SharedPtr> subs_;
    std::map<std::string, sensor_msgs::msg::Range::SharedPtr> latest_msgs_;
...
#endif // TERMINAL_RVIZ_DISPLAYS_RANGE_DISPLAY_HPP_
